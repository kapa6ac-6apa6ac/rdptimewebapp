    ...
    <v-row no-gutters style="flex-wrap: nowrap">
      ...
      >
        ...
        >
          ...
          <template v-slot:item.timeRDP="{ item }">{{
            secondsToTime(item.timeRDP)
          }}</template>
          <template v-slot:item.timeSCUD="{ item }"
          ...
          >
          <template v-slot:item.timeALL="{ item }">{{
            secondsToTime(item.timeRDP + item.timeSCUD)
          }}</template>
          <template v-slot:item.timeVector="{ item }">{{
            secondsToTime(item.timeVector)
          }}</template>
          <template v-slot:item.timeManic="{ item }">{{
            secondsToTime(item.timeManic)
          }}</template>
        </v-data-table>
      </v-col>
      <v-col
      ...
      >
        ...
        <v-checkbox
          v-model="loadManicTime"
          @change="loadData()"
          label="Загрузка времени из ManicTime (очень долго)"
        ></v-checkbox>
        <v-btn @click="dlReport()" color="primary" block class="mt-2"
          >Скачать отчет за {{ date }}</v-btn
        >
      </v-col>
    </v-row>
...
export default Vue.extend({
  ...
  methods: {
    ...
    loadData() {
      ...
      this.data = [];
      fetch("/api/time/" + date + "?timeManic=" + this.loadManicTime)
    ...
    dlReport() {
      const date = this.date.replace("-", "/").replace("-", "/");
      if (this.loadManicTime)
        window.open("/api/report/" + date + "?manicTime=true", "_blank");
      else window.open("/api/report/" + date, "_blank");
    },
    ...
    customSort(items: any, index: string[], isDesc: boolean[]) {
      items.sort((a: any, b: any) => {
        if (index[0] === "timeALL") {
          if (!isDesc[0]) {
            return a["timeRDP"] + a["timeSCUD"] < b["timeRDP"] + b["timeSCUD"]
              ? -1
              : 1;
          } else {
            return b["timeRDP"] + b["timeSCUD"] < a["timeRDP"] + a["timeSCUD"]
              ? -1
              : 1;
          }
        } else {
  ...
  computed: {
    ...
    dataHeaders() {
      ...
      if (this.loadManicTime)
        headers.push({ text: "Время ManicTime", value: "timeManic" });
